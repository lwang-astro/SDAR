...
# Check binary exists
if ! command -v ~/bin/ar.logh.sd >/dev/null 2>&1; then
    echo "Error: ~/bin/ar.logh.sd not found. Run 'make -C sample/AR && make -C sample/AR install' first." >&2
    exit 1
fi

echo "# $(date): ~/bin/ar.logh.sd -t 1.0e-3 -n 1000 -G 1.0 $INPUT" >> commands.log
...
~/bin/ar.logh.sd -t 1.0e-3 -n 1000 -G 1.0 "$INPUT" > triple.logh.sd.log

echo "Done. Output: $WORKDIR/triple.logh.sd.log"
