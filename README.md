...
2. AR: a _N_-body integrator using AR library. Sample executables are generated by `make -C sample/AR`, named as `ar.<method>[.ttl][.sd][.kdkpert][.cm][.mpfrc]`. The meaning of the name segments:
    1. `<method>` indicates the g-function form: `logh` (standard LogH), `blogh`, `normblogh`, `mulall`, `btlogh`, `maxpot` or `addpot` (see `docs/hierarchical_blogh_impl_notes.md`).
    2. `ttl` indicates the Time-Transformed Leapfrog implementation of the method (absent = LogH implementation).
    3. `sd` indicates the tree-based hierarchical slowdown method for inner and outer binaries in a hierarchical system.
    4. `kdkpert` indicates the KDK perturbation splitting; `cm` the center-of-mass frame build; `mpfrc` the arbitrary-precision (MPFR) build.
    Examples: `ar.logh`, `ar.logh.ttl.sd`, `ar.blogh.ttl.sd.cm`, `ar.logh.sd.kdkpert`, `ar.logh.mpfrc`.
